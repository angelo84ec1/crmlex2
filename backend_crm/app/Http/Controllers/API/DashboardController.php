    ...
    if ($user->role == 'Cliente' || $user->role == 'Digitador' || $user->role == 'Assistant' || $user->role == 'Supervisor') {
        ...
        foreach ($projects as $project) {
            ...
            $data1 = [
                ...
                'start' => Carbon::parse($project->project_startdate)->format('Y-m-d'),
                'end' => Carbon::parse($project->project_enddate)->format('Y-m-d'),
                ...
                'styles' => [
                    ...
                    'progressSelectedColor' => ($project->project_status === 'completed') ? '#20BF55' :
            ...
            foreach ($project->tasks as $key=>$task) {
                ...
                $taskData = [
                    'type' => 'project',
                    'id' => Time().$project->id.$key,
                    'name' => $task->taskName->title . " (" . $assignUsers . ")",
                    'start' => Carbon::parse($task->taskName->start_date)->format('Y-m-d'),
                    'end' => Carbon::parse($task->taskName->end_date)->format('Y-m-d'),
                    ...
                    'styles' => [
                        'backgroundColor' => '#CFD8D7',
                        'progressSelectedColor' => ($task->status === 'completed') ? '#20BF55' :
                            (($task->status === 'new') ? '#8E2DE2' :
                                (($task->status === 'late') ? '#CD500C' :
                                    (($task->status === 'pending') ? '#D9BB41' : '#4A00E0'))),
                    ],
                ];
                ...
                foreach ($task->taskName->subTask as $key=>$stask) {
                    ...
                    $subTaskData = [
                        'type' => 'project',
                        'id' => $project->id.$key.time(),
                        'name' => $stask->title . " (" . $assignsubUsers . ")",
                        'start' => Carbon::parse($stask->start_date)->format('Y-m-d'),
                        'end' => Carbon::parse($stask->end_date)->format('Y-m-d'),
                        'progress' => $stask->progress ?? 0,
                        'detests' => $currentDateTime,
                        'statusdelte' => $stask->status,
                        'styles' => [
                            'backgroundColor' => '#CFD8D7',
                            'progressSelectedColor' => ($stask->status === 'completed') ? '#20BF55' :
                                (($stask->status === 'new') ? '#8E2DE2' :
                                    (($stask->status === 'late') ? '#CD500C' :
                                        (($stask->status === 'pending') ? '#D9BB41' : '#4A00E0'))),
                        ],
                    ];
                    $data[] = $subTaskData;
                }
