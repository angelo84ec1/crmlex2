        ...
        >
          <Box
            sx={{
              ...
              cursor: "pointer",
              backgroundColor:
                row.item?.task_name?.status === "completed"
                  ? "#20BF55"
                  : row.item?.task_name?.status === "new"
                  ? "#8E2DE2"
                  : row.item?.task_name?.status === "late"
                  ? "#CD500C"
                  : row.item?.task_name?.status === "pending"
                  ? "#D9BB41"
                  : "#4A00E0",
            }}
          >
          ...
          <IconButton
            ...
            onClick={() => {
              ...
              dispatch(
                setCurrentTask({
                  ...row.item?.task_name,
                  project_id: row.item?.project_id,
                })
              );
              editTask();
            }}
              ...
              <Table size="small" aria-label="purchases">
                        ...
                        sx={{
                          ...
                          display:
                            ...
                            row.subtasks.length === 0
                              ? "show"
                              : row && row.subtasks && row.subtasks.length > 0
                              ? "none"
                              : "",
                        }}
                        onClick={() => {
                ...
                <TableBody>
                  ...
                  {row.item?.sub_tasks &&
                    row.item?.sub_tasks
                      .filter(
                        (tasksRow) => tasksRow.task_id === row.item?.task_id
                      )
                      .map((tasksRow, i) => (
                          ...
                          <TableCell
                            ...
                            onClick={() => {
                              if (
                                ...
                                dispatch(
                                  setCurrentTask(tasksRow?.sub_task_name)
                                );
                                editSubTaskStatus();
                              }
                            }}
                          >
                            <Box
                              sx={{
                                ...
                                borderRadius: 2,
                                backgroundColor:
                                  tasksRow?.sub_task_name?.status ===
                                  "completed"
                                    ? "#20BF55"
                                    : tasksRow?.sub_task_name?.status === "new"
                                    ? "var(--secondary-color)"
                                    : tasksRow?.sub_task_name?.status === "late"
                                    ? "#CD500C"
                                    : tasksRow?.sub_task_name?.status ===
                                      "pending"
                                    ? "#D9BB41"
