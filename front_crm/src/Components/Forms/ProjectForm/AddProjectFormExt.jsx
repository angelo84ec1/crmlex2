...
import { addTasks, resetTask } from "../../../Redux/taskReducer";
import React, { useState } from "react";

import CheckIcon from "@mui/icons-material/Check.js";

export default function AddProjectFormExt() {

    const theme = useTheme()
    const { user } = useSelector(state => state.Auth)
    const { status, currentProject, projectTasks } = useSelector(state => state.TaskReducer)
    ...
    const schema = yup.object({
        ...
        sub_task: yup.array().min(1, 'Can`t be empty')
    }).required();
    ...
    const onSubmit = data => {
        // subtasks with only title like ["task1", "task2"]
        const subt = [];
        data.sub_task.forEach((item) => {
            subt.push(item.title);
        });
        const newData = {
            customer: data.customer,
            startDate: data.startDate,
            title: data.task[0].title,
            discription: "Adding Task in client",
            extend_days: data.extend_days
        }
        console.log(newData, 'newData')

        const givenDate = new Date(setDate(data.startDate));
        const givenDueDate = new Date(setDate(data.dueDate));
        ...
        dispatch(
            addTasks({
                ...newData,
                sub_task: subt,
                subtaskStartDate: dateFormat(givenDate),
                subtaskDueDate: data.dueDate === null ? dateFormat(oneYearAfter) : dateFormat(givenDueDate),
                user_id: user.user_id,
                startDate: dateFormat(givenDate),
                dueDate: data.dueDate === null ? dateFormat(oneYearAfter) : dateFormat(givenDueDate),
